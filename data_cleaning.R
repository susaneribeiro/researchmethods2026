...
df_clean <- df_clean %>%
  mutate(
    total_pretest  = pretest1 + pretest2 + pretest3 + pretest4 + 
                     pretest5 + pretest6 + pretest7 + pretest8,
    total_posttest = (posttest1 + posttest5 + posttest6 + posttest7) * 6 +
                     (posttest2 + posttest3 + posttest4 + posttest8) * 8,
    # Normalized versions (0 to 100)
    pretest_pct  = (total_pretest  / 8)  * 100,
    posttest_pct = (total_posttest / 56) * 100
  )
...
# ============================================================
# 4. CENTERING the predictors
# ============================================================

df_clean <- df_clean %>% 
  mutate(
    pretest_pct_c = pretest_pct - mean(pretest_pct),
    total_pretest_c = total_pretest - mean(total_pretest),
    score_spaced_c = score_spaced - mean(score_spaced),
    score_retrieval_c = score_retrieval - mean(score_retrieval),
    score_interl_c = score_interleaved - mean(score_interleaved)
  )
